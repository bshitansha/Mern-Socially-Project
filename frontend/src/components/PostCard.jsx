...
function PostCard({ post, refreshPosts }) {
  ...
  const isOwner = user && post.author?._id?.toString() === user.id;

  const hasLiked =
    user && post.likes?.some((likeId) => likeId.toString() === user.id);
  ...
  return (
    <article className="post-card">
      ...
      <div className="post-stats">
        <button
          className={hasLiked ? "stat-button liked" : "stat-button"}
          onClick={handleLike}
          disabled={actionLoading}
        >
          ♥ {post.likes?.length || 0}
